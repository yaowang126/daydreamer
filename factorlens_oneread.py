# -*- coding: utf-8 -*-
"""
Created on Tue Jun 27 09:20:31 2023
...
class Factorlens:
    ...
    def __init__(self,factor_name,factor_df,stock_pool=None,last_date=None):
        ...
        self.factor_date_list = factor_df.date.unique().tolist()
        self.trade_cal = self.selector.trade_cal(start_date=min(self.factor_date_list), 
        ...
        self.factor_df = pd.merge(left=factor_df,right=trade_date_df.loc[:,['factor_date','nexttrade_date']],
                         left_on='date',right_on='factor_date',how='left')
    ...
    def backtest(self,method = 'buyonlysellable',layer_num=10,keep_null=True):
        assert method in ('buyonlysellable','somethingelse'),'invalid method'
        if method == 'buyonlysellable':
            for i in range(len(self.date_list)-1):             
                trade_date = self.date_list[i]
                trade_date_next = self.date_list[i+1]
    ...
    def draw(self,path=None):
        ...
        axes1.set_title(f'ic={round(self.metrics_df.ic.mean(),4)}')
